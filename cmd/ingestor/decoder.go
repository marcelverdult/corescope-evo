...
// the server via the meshdecode module — they decode identically in both
// binaries. They are re-exported here as aliases/consts so the rest of the
// ingestor package (and its tests) keep using the original local names.
//
...
// local: the ingestor Payload carries extra channel-decryption and telemetry
// fields the server does not. The channel-decryption helpers and decodeHeader
// (which resolves names through the ingestor-local payloadTypeNames map) are
// also ingestor-only.
