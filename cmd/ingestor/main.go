...
// the MQTT envelope's "timestamp" field. Falls back to ingest time when the
// field is missing, unparseable, or implausibly in the future (a clock-skewed
// observer). Result is always RFC3339 UTC.
//
...
func resolveRxTime(msg map[string]interface{}, tag string) string {
	...
	// timestamps that would show a warning in the UI for every packet from
	// UTC+N nodes.
	if t.After(now) {
		return now.Format(time.RFC3339)
	}
